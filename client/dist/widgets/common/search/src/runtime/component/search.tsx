...
import {
  ...
  AppMode, RecordSetChangeType, MessageManager, DataRecordsSelectionChangeMessage, DataSourceFilterChangeMessage, hooks, DataRecordSetChangeMessage
} from 'jimu-core'
import { TextInput, Button, Link, type LinkTarget, defaultMessages as jimuiDefaultMessage } from 'jimu-ui'
import { type IMConfig, type Suggestion, SearchResultView, type ServiceList, type InitResultServiceListOption, SearchServiceType, type IMServiceList, type IMSearchResult, type IMSelectionList } from '../../config'
...
const SearchInput = (props: SearchSettingProps) => {
  ...
  const isShowLocationRef = useRef<boolean>(false)
  ...
  const [isOpenSuggestionPopper, setIsOpenSuggestionPopper] = useState(false)
  ...
  useEffect(() => {
    setIsOpenSuggestionPopper(isOpenSuggestion && !isShowLoading && !isOpentResultPopper)
  }, [isOpenSuggestion, isShowLoading, isOpentResultPopper])
  ...
  */
  const onSearchInputFocus = (e) => {
    if (isFocusSuggestion.current || isShowLocationRef.current) return false
    const value = e?.target?.value
  ...
  */
  const onSuggestionItemClick = (searchText: string, initResultServiceListOption?: InitResultServiceListOption, isUseLocationError?: boolean) => {
    if (isUseLocationError) {
      isShowLocationRef.current = true
  ...
  */
  ...
  const checkIsOpenSuggestionPopper = () => {
    ...
    if (config?.maxSuggestions > 0) {
      isOpen = isOpenSuggestionPopper && isHasServiceSupportSuggest
    } else {
      const isShowCurrentLocationOrRecentSearch = (!searchValue && config?.isUseCurrentLoation) || isRecentSearches
      isOpen = isOpenSuggestionPopper && isShowCurrentLocationOrRecentSearch
    }
    if (!isOpen) {
      isFocusSuggestion.current = false
      isShowLocationRef.current = false
    }
    return isOpen
  }
  ...
  const handleInputBlur = (e) => {
    setTimeout(() => {
      if (!isFocusSuggestion.current && !isShowLocationRef.current) {
        setIsOpenSuggestion(false)
      }
    }, 200)
  }
