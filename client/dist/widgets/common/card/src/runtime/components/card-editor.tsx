...
> {
  ...
  handleBreakLink = evt => {
    ...
    if (builderStatus === Status.Regular) {
      otherLayoutId = appConfig.widgets[widgetId].layouts[Status.Hover][browserSizeMode]
    } else {
      otherLayoutId = appConfig.widgets[widgetId].layouts[Status.Regular][browserSizeMode]
    }
    action.removeLayoutItem(
    ...
    //After `duplicateLayoutItems`, the structure of the layout is wrong, we need to call the `buildLayoutStructure` method again to reconstruct it
    action.clearLayoutStructure(otherLayoutId, browserSizeMode, true)
    action.buildLayoutStructure(otherLayoutId, browserSizeMode, true)

    action.clearLayoutStructure(currentLayoutId, browserSizeMode, true)
    action.buildLayoutStructure(currentLayoutId, browserSizeMode, true)
