...
> {
  ...
  onHoverLayoutOpenChange = evt => {
    ...
    if (config[Status.Hover].enable && !value) {
      ...
      const oriLayoutId = appConfig.widgets[id].layouts[Status.Regular][browserSizeMode]
      const desLayoutId = appConfig.widgets[id].layouts[Status.Hover][browserSizeMode]

      desLayoutId && action.removeLayout(desLayoutId, true)
      const newHoverLayout = action.duplicateLayout(oriLayoutId, false)
      action.editLayoutLabel({ layoutId: newHoverLayout.id }, this.formatMessage('hover'))
