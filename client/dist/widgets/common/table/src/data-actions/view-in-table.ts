import { AbstractDataAction, type DataRecordSet, utils, getAppStore, appActions, MutableStoreManager, DataSourceTypes, type UseDataSource, DataSourceStatus } from 'jimu-core'
...
export default class ViewInTable extends AbstractDataAction {
  ...
  async onExecute (dataSet: DataRecordSet, actionConfig?: any): Promise<boolean> {
    ...
    const isAllRecords = records?.length === 0
    const name = (isAllRecords ? '' : dataSet.name) || dataSource.getLabel() || dataSource.getDataSourceJson()?.sourceLabel
    const useDataSource = {
