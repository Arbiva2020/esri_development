...
> {
  ...
  editorLayerWatcher = (event, id, layerName) => {
    const { dataSources } = this.state
  ...
  createEditor = (mapView?: JimuMapView) => {
    ...
    const newEditor = () => {
      ...
      reactiveUtils.watch(() => this.edit.viewModel.state, editState => {
        if (editState === 'editing-existing-feature') {
          const feature = (that.edit.viewModel as any)?.featureFormViewModel?.feature
          if (!feature) return
          const { dataSources, selectionStartWorkflow } = that.state
          const curLayerId = feature.layer.id
