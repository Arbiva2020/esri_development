/** @jsx jsx */
import { React, jsx, css, SupportedItemTypes as JimuSupportedItemTypes, Immutable, type DataSourceJson, dataSourceUtils, ReactRedux, type IMState, DataSourceStatus, ServiceManager, SupportedLayerServiceTypes, type ImmutableArray } from 'jimu-core'
...
export const DataItemSearch = (props: DataItemSearchProps) => {
  ...
  const shouldBeDisplayedItemCategoriesInfo = useMemo(() => {
    return itemCategoriesInfo ? itemCategoriesInfo.filter((item) => item.enabled).map((item) => ({ type: item.type, customLabel: item.customLabel || '', curatedFilter: item.curatedFilter })).asMutable() : undefined
  }, [itemCategoriesInfo])
