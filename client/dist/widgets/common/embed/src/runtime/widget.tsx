...
interface State {
  ...
  // Indicates the code is not socail media
  useSrcdoc?: boolean
}
...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & Props, State> {
  ...
  static mapExtraStateProps = (
    ...
    const createdTime = state?.appInfo?.created
  ...
  constructor (props) {
    ...
    if (!isOldApp) {
      ...
      useEmbedCode = mediaCodeOrSanitizedHtml
    }
    const state: State = {
      content:
      ...
      useSrcdoc: isOldApp ? true : !embedByCodeUrl
    }
    this.state = state
  ...
  componentDidUpdate (preProps, preStates) {
    ...
    const { appMode, config, id, isOldApp } = this.props
    ...
    const useSrcdoc = isOldApp ? true : !embedByCodeUrl
  ...
  loadContent = () => {
    const { config, isOldApp } = this.props
    ...
    if (this.ifr) {
      ...
      if (embedType === EmbedType.Code) {
        if (useSrcdoc) {
          ...
          const useContent = isOldApp ? `${docSafeString}${content}` : content
          this.ifr.srcdoc = useContent
        } else {
          setTimeout(() => {
  ...
  render () {
    ...
    const { theme, id, config, isOldApp } = this.props
