import {
  AbstractDataAction, type DataRecordSet, getAppStore, type ImmutableObject, type JimuMapViewInfo, DataSourceStatus,
  MutableStoreManager, utils, i18n
} from 'jimu-core'
import { MapViewManager, SHOW_ON_MAP_DATA_ID_PREFIX, ActionType, zoomToUtils, featureUtils } from 'jimu-arcgis'
...
export default class ShowOnMap extends AbstractDataAction {
  ...
  async onExecute (dataSet: DataRecordSet, actionConfig?: any): Promise<boolean> {
    ...
    if (jimuMapView && featureSet?.features?.length > 0) {
      zoomToUtils.zoomTo(jimuMapView?.view, featureSet.features, {
        padding: {
          left: 50,
          right: 50,
          top: 50,
