...
import {
  ...
  isTwoWidgetsMutuallyExtentChange
} from '../utils'
...
export default class MapBase extends React.PureComponent<Props, State> {
   ...
   */
  tryUpdateCurrentViewByConfigProps (): void {
    if (!this.view) {
      return
    }
  ...
  async createWebMapOrWebSceneByDataSource (dataSourceInfo: MapbaseDataSourceInfo): Promise<__esri.WebMap | __esri.WebScene> {
    ...
    if (dataSourceUtils.getWhetherUseProxy()) {
      ...
      const tables = (webMapOrWebScene as any).tables ? (webMapOrWebScene as any).tables.toArray() : []
      ...
      webMapOrWebScene.allLayers.toArray().forEach(layer => { // set proxy url for layers
    ...
    if (relatedWidgetIds && relatedWidgetIds.includes(this.props.baseWidgetProps.id)) {
      ...
      if (zoomToFeatureInternalValue.type === 'zoom-to-extent') {
        ...
        layerPromise.then(layer => {
          ...
          processZoomToFeatures(mapBaseView, target.layer, (target && target.graphics) ? target.graphics : target).then(
            graphics => {
              if (layer) {
  ...
  handleActionForPanToActionValue (mutableStateProps: ActionRelatedProps, mapBaseView: MapbaseView): void {
    ...
    if (relatedWidgetIds && relatedWidgetIds.includes(this.props.baseWidgetProps.id)) {
      ...
      const panToValue = mutableStateProps.panToActionValue.value
      const dataSource = this.dsManager.getDataSource(panToValue.dataSourceId) as unknown as JSAPILayerMixin
      let layerPromise
      ...
      layerPromise.then(layer => {
        zoomToUtils.layerExtent(mapBaseView, layer).then(layerExtent => {
          if (layerExtent) {
