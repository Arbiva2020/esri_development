...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig> & ExtraProps, IState> {
          ...
          setTimeout(() => {
            ...
            if (this.widgetOpenedWithSelection) {
              ...
              if (currentWidgetState === WidgetState.Opened && this.props.mutableStateProps?.selectedFeatureRecords?.length > 0) {
                ...
                if (isFeatureValid) {
                  //when feature is selected don't loadSelectOrDrawTool on load as it may change few states which don't render the chart at all
    ...
    return new Promise((resolve) => {
      ...
      if (this._defaultViewModel?.input) {
        ...
        if (this.selectedBufferValues.enabled && this.selectedBufferValues.bufferDistance > 0) {
          let inputGeometry = this._defaultViewModel.input.geometry.clone()
          ...
          if (!geometryEngine.isSimple(inputGeometry)) {
  ...
  */
  ...
  getSelectableLayers = (activeDs: string) => {
    ...
    dataSource?.forEach((layer) => {
      ...
      if (eachLayer && eachLayer.layerDefinition && eachLayer.layerDefinition.geometryType) {
        ...
        if (isSelectableLayerCustomized) {
  ...
  /**
   * Check features availability selected from the other widgets in the active data source
  */
  ...
  componentDidUpdate = (prevProps) => {
    ...
    if (this.didProfileLayersSettingsChanged(prevConfig?.profileSettings.layers, currentConfig?.profileSettings.layers) ||
      ...
      this.setState({
        ...
        if (currentConfig?.profileSettings.isProfileSettingsEnabled && !currentConfig?.profileSettings.isCustomizeOptionEnabled &&
           this.state.profileLineLayers.length === 0 && !this.state.drawModeActive && !this.state.selectModeActive) {
          this.onBackClick()
