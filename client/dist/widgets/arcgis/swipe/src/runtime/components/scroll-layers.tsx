...
export function ScrollLayers (props: ScrollLayersProps) {
   ...
   */
  const { widgetId, activeMapView, inactiveMapView, firstTrailingLayersId, secondTrailingLayersId, swipeStyle, handleScrollingLayerOfFirstMapView, handleScrollingLayerOfSecondMapView, handleScrollFirstMapView, dividerColor, isDesignMode } = props
  ...
  const mapContainerRef = useRef<HTMLElement>(document.querySelector<HTMLElement>(`div[data-widgetid=${mapId}] .widget-content`))
  const getMultiMapContainer = (mapContainerRef, mapId) => {
    return mapContainerRef.current.querySelector(`div[data-widgetid=${mapId}] .multi-map-container`)
  }
  const multiMapContainerRef = useRef<HTMLElement>(getMultiMapContainer(mapContainerRef, mapId))
  ...
  useEffect(() => {
    mapContainerRef.current.style.pointerEvents = isDesignMode ? 'none' : null
  }, [isDesignMode])

  useEffect(() => {
    return () => {
      // eslint-disable-next-line react-hooks/exhaustive-deps
      mapContainerRef.current.style.pointerEvents = null
    }
  }, [])
  ...
  useEffect(() => {
    const restoreListener = () => {
      multiMapContainerRef.current = getMultiMapContainer(mapContainerRef, mapId)
  ...
  useEffect(() => {
     ...
     */
    if (activeMapView?.view && mapContainerRef.current) {
      initScrollerContent()
    ...
    function scrollMaps () {
      ...
      const handleScrollMapsVertical = () => {
        const scrollerMap = mapContainerRef.current.querySelector(`#${scrollerId}`)
      ...
      const initScrollerMapContent = () => {
        const scrollerMap = mapContainerRef.current.querySelector<HTMLElement>(`#${scrollerId}`)
    ...
    function secondScroll () {
      const initScrollerContentSecond = () => {
        const scrollerSecond = mapContainerRef.current.querySelector<HTMLElement>(`#${scrollerId}`)
        const contentSecond = mapContainerRef.current.querySelector<HTMLElement>(`#${contentId}`)
        ...
        if (!tickingSecond.current) {
          requestAnimationFrame(() => {
            ...
            swipesSecondRef?.current?.forEach((swipe, index, swipes) => {
              ...
              swipe.when(() => {
                const dividers = mapContainerRef.current.querySelectorAll<HTMLElement>('.esri-swipe__divider')
    ...
    function initScrollerContent () {
      ...
      mapContainerRef.current.appendChild(scroller)
    ...
    function destroyFirstScroll () {
      const scroller = mapContainerRef.current.querySelector(`#${scrollerId}`)
      scroller && mapContainerRef.current.removeChild(scroller)
      swipesFirstRef?.current?.forEach((swipe, index, swipes) => {
        swipe.position = 0
        swipe.leadingLayers.removeAll()
        swipe.trailingLayers.removeAll()
    ...
    function destroyScrollMaps () {
      ...
      const scrollerMap = mapContainerRef.current.querySelector('.scrollerMap')
      scrollerMap && mapContainerRef.current.removeChild(scrollerMap)
      clearMapViewStyle()
      if (dividerDomRef.current && mapContainerRef.current.querySelector(`#${sliderId}`) && multiMapContainerRef.current.contains(dividerDomRef.current)) {
  ...
  }, [widgetId, view, activeMapView, firstTrailingLayersId, secondTrailingLayersId, swipeStyle, dividerColor, sliderId, viewHeight, viewWidth, scrollerId, contentId, inactiveMapView])
