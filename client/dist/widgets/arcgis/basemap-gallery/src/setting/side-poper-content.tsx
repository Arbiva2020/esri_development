...
import { type IMConfig, type basemapItem } from '../config'
...
const SidePopperContent = (props: SettingProps): React.ReactElement => {
  ...
  const initGroups = async () => {
    ...
    const esriDefaultGroup = await getBasemapGroupByDefault(esriRequest.current, portalUrl.current, portalSelf.current, token, true)
    const orgDefaultGroup = await getBasemapGroupByDefault(esriRequest.current, portalUrl.current, portalSelf.current, token, false)
  ...
  const refreshItemsByGroupId = async (groupId) => {
    const newItems = await getBasemapItemsByGroupId(portal.current, portalUrl.current, groupId, false)
  ...
  const onItemSelected = (item: basemapItem, isSelected: boolean) => {
    let items
    if (isSelected) {
