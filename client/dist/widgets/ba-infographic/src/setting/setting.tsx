...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<any>, any> {
  ...
  reportCheckedHandler (ev: any) {
    ...
    const newPublicList = reportList.public.map(item => {
      const matchingReport = report.find(r => r.id === item.id)
      if (matchingReport) {
  ...
  componentDidUpdate (prevProps, prevState) {
    const { widgetMode, id, sourceCountry, selectedGeographyLevels, baSearchType, viewMode } = this.props.config
  ...
  render () {
    const { selectedCountry, activeGeographyLevels, geographiesChecked, pointsOfInterestChecked, presetBuffersQueued, maxDriveBuffer, maxWalkBuffer, stViewMode } = this.state
    const { reportList, availableGeographyLevels, selectedGeographyLevels, drawPointEnabled, drawPolygonEnabled } = this.props.config
    const { widgetMode, backgroundColor, runReportOnClick, imageExport, dynamicHtml, excel, pdf, print, fullscreen, zoomLevel, displayHeader, headerColor, headerTextColor, sourceCountry } = this.props.config
